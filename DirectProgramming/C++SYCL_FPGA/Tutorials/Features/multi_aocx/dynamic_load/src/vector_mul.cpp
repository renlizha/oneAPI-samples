...
void VectorMulInDPCPP(queue q, const IntArray &addend_1,
  ...
  buffer<int, 1> mul_buf(sum_parallel.data(), num_items);
  ...
  q.submit([&](handler &h) {
    ...
    // the mul_accessor is used to store (with write permision) the sum data
    auto mul_accessor = mul_buf.get_access<dp_read_write>(h);
